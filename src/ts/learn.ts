...
interface LessonSection {
  ...
  body: string;
  keyTerms: string[];
  keyTermsDefs?: Record<string, string>;
}
...
const subjectCache: Map<string, SubjectFull> = new Map();
...
let currentLessonId = "";
let currentLesson: LessonFull | null = null;
...
function setupCopyProtection() {
  ...
  document.addEventListener("copy", (e) => {
    if (!readerView.classList.contains("hidden")) e.preventDefault();
  });
...
async function loadManifest(): Promise<Manifest> {
  ...
  if (!res.ok) throw new Error("Failed to load lesson manifest");
  return res.json();
}

async function loadSubjectData(file: string): Promise<SubjectFull> {
  if (subjectCache.has(file)) return subjectCache.get(file)!;
...
function highlightKeyTerms(body: string, keyTerms: string[]): string {
  if (!keyTerms.length) return escapeHtml(body);
...
function renderSection(lesson: LessonFull, sectionIndex: number) {
  ...
  (prevSectionBtn as HTMLButtonElement).disabled = sectionIndex === 0;
  (nextSectionBtn as HTMLButtonElement).disabled = sectionIndex === lesson.sections.length - 1;

  // Setup tooltips
  setupKeyTermTooltips(section);
}

function setupKeyTermTooltips(section: LessonSection) {
  const defs = section.keyTermsDefs || {};
  const terms = lessonContent.querySelectorAll(".key-term, .key-term-pill");

  terms.forEach((term) => {
    const termText = (term as HTMLElement).dataset.term || term.textContent?.trim() || "";
...
async function showReader(lessonId: string, file: string) {
...
function navigateSection(direction: number) {
  ...
  if (newIndex < 0 || newIndex >= currentLesson.sections.length) return;
...
function updateCompleteUI() {
  ...
  (completeBtn as HTMLButtonElement).disabled = done;
  completeStatus.classList.toggle("hidden", !done);
}

// ---- Events ----
subjectTabs.addEventListener("click", (e) => {
  const btn = (e.target as HTMLElement).closest(".subject-tab") as HTMLElement | null;
  if (!btn) return;
...
lessonGrid.addEventListener("click", (e) => {
  const btn = (e.target as HTMLElement).closest(".read-btn") as HTMLElement | null;
  if (!btn) return;
  const lessonId = btn.dataset.lesson || "";
  const file = btn.dataset.file || "";
  if (lessonId && file) showReader(lessonId, file);
});
...
completeBtn.addEventListener("click", async () => {
  ...
  // Sync to Firebase
  const account = (window as any).atomAccount;
  if (account?.getUser?.() && account.saveLearnProgress) {
    try {
      await account.saveLearnProgress([...completedLessons]);
    } catch {}
  }
