(() => {
  ...
  function ensureSettingsQuickLink() {
    const nav = document.querySelector('.nav');
    if (!nav) return null;
    const link = nav.querySelector('a[href*="settings.html"]') as HTMLAnchorElement | null;
    if (!link) return null;
  ...
  async function bindAccountNav() {
    ...
    if (navMoreWrap && navMoreWrap.parentElement) {
      ...
      if (accountLink) controls.appendChild(accountLink);
      if (settingsLink) controls.appendChild(settingsLink);
